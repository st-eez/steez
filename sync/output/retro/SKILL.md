...
preamble-tier: 2
version: 2.0.0
description: Weekly engineering retrospective. Analyzes commit history, work patterns, and code quality metrics with persistent history and trend tracking. Team-aware: breaks down per-person contributions with praise and growth areas. Use when asked to "weekly retro", "what did we ship", or "engineering retrospective". Proactively suggest at the end of a work week or sprint. (steez)
allowed-tools:
  - Bash
  - Read
  - Write
...
```
| Skill Usage | /steez-ship(12) /qa(8) /review(5) · 3 safety hook fires |
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.steez/steez-retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.steez/steez-retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.steez/steez-retros/global-${today}-${next}.json`:
...
5. Save only the current-window snapshot to `.context/steez-retros/` (same as a normal retro run); do **not** persist the prior-window metrics.
...
- Output directly to the conversation — do NOT write to filesystem (except the `.context/steez-retros/` JSON snapshot)

## Important Rules

- ALL narrative output goes directly to the user in the conversation. The ONLY file written is the `.context/steez-retros/` JSON snapshot.
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.steez/steez-retros/` (not `.context/steez-retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
