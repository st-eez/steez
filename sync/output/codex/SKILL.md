...
```bash
TMPERR=$(mktemp /tmp/steez-codex-err-XXXXXX.txt)
```
...
```bash
cat .context/steez-codex-session-id 2>/dev/null || echo "NO_SESSION"
```
...
2. Create temp files:
```bash
TMPRESP=$(mktemp /tmp/steez-codex-resp-XXXXXX.txt)
TMPERR=$(mktemp /tmp/steez-codex-err-XXXXXX.txt)
```
...
to `.context/steez-codex-session-id`.
