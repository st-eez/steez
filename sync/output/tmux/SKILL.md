...
echo '{"skill":"steez-tmux","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> "$STEEZ_HOME/analytics/skill-usage.jsonl" 2>/dev/null || true
```
## Beads Context
...
At the end of each major workflow step, rate your /steez-tmux experience 0-10. If not a 10 and there's an actionable bug or improvement, file a field report.
...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-tmux
```
...
echo '{"skill":"steez-tmux","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
