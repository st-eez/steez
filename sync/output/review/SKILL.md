...
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "steez-review" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.steez/analytics/eureka.jsonl 2>/dev/null || true
```
## Skill Self-Report
...
# Compute project slug for ~/.steez/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".steez/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
```bash
source <(~/.steez/bin/steez-diff-scope <base> 2>/dev/null) || true
...
1. **Testing** — read `~/.steez/repo/review/specialists/testing.md`
2. **Maintainability** — read `~/.steez/repo/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.steez/repo/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.steez/repo/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.steez/repo/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.steez/repo/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.steez/repo/review/design-checklist.md`
...
```bash
~/.steez/bin/steez-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.steez/repo/review/specialists/red-team.md`
...
```bash
TMPERR_ADV=$(mktemp /tmp/steez-codex-adv-XXXXXXXX)
...
If Codex is NOT available: "Codex CLI not found — running Claude adversarial only. Install Codex for cross-model coverage: `npm install -g @openai/steez-codex`"
...
```bash
TMPERR=$(mktemp /tmp/steez-codex-review-XXXXXXXX)
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
...
```bash
~/.steez/bin/steez-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.steez/bin/steez-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
