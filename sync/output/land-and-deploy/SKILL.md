...
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. Merges the PR, waits for CI and deploy, verifies production health via canary checks. Takes over after /steez-ship creates the PR. Use when: "merge", "land", "deploy", "merge and verify", "land it", "ship it to production". (steez)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "steez-land-and-deploy" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.steez/analytics/eureka.jsonl 2>/dev/null || true
```
## Skill Self-Report

At the end of each major workflow step, rate your /steez-land-and-deploy experience 0-10. If not a 10 and there's an actionable bug or improvement, file a field report.
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /land-and-deploy is not yet implemented. Run `/steez-ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.
...
```bash
cat ~/.steez/repo/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
**If B:** **STOP.** "Good call — run `/steez-review` for a thorough pre-landing review. When that's done, run `/land-and-deploy` again and I'll pick up right where we left off."
...
- If reviews are stale: "Run `/steez-review` or `/steez-autoplan` to review the current code, then `/land-and-deploy` again."
