...
import { resolveConfig, ensureStateDir, readVersionHash, getGitRoot, getRemoteSlug } from '../config';
...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.steez'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.steez', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.steez/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.steez');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.steez', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.steez');
    ...
    test('adds .steez/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.steez/');
      expect(content).toBe('node_modules/\n.steez/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .steez/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.steez/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.steez', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .steez/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
...
describe('resolveServerScript', () => {
  ...
  const { resolveServerScript } = require('../cli');
  ...
  test('finds server.ts adjacent to cli.ts in dev mode', () => {
    ...
    const result = resolveServerScript({}, coreDir, '');
    expect(result).toBe(path.join(coreDir, 'server.ts'));
  });
...
describe('isServerHealthy', () => {
  const { isServerHealthy } = require('../cli');
