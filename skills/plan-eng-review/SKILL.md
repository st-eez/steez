...
if [ -n "$_ENG_BEAD" ]; then
  _BEAD_DESIGN_DOC=$(bd show "$_ENG_BEAD" --json 2>/dev/null | jq -r '.[0].notes // ""' | grep '^Design doc:' | sed 's/^Design doc: //')
  _BEAD_BRANCH=$(bd show "$_ENG_BEAD" --json 2>/dev/null | jq -r '.[0].notes // ""' | grep '^Branch:' | sed 's/^Branch: //')
  echo "Review bead: $_ENG_BEAD"
...
SLUG=$(~/.steez/bin/slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
If a design doc exists (from `/office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design — check the prior version for context on what changed and why.
...
4. **Search check:** For each architectural pattern, infrastructure component, or concurrency approach the plan introduces:
   ...
   If the plan rolls a custom solution where a built-in exists, flag it as a scope reduction opportunity. Annotate recommendations with **[Layer 1]**, **[Layer 2]**, **[Layer 3]**, or **[EUREKA]** (see `~/.steez/repo/ETHOS.md` for the full framework). If you find a eureka moment — a reason the standard approach is wrong for this case — present it as an architectural insight.
...
**Persist the result:**
```bash
~/.steez/bin/review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Additional rules for plan review questions:
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `~/.steez/repo/skills/review/TODOS-format.md`.
...
If a bead with label `skill:eng-review` is active (check via `bd list --status=in_progress`), hand off the review results:

```bash
# Hand off eng review results to the bead (non-blocking)
_ENG_BEAD_ID=$(bd list --status=in_progress --json 2>/dev/null | python3 -c "import sys,json; beads=json.load(sys.stdin); print(next((b['id'] for b in beads if 'eng-review' in b.get('title','').lower()), ''))" 2>/dev/null || echo "")
if [ -n "$_ENG_BEAD_ID" ]; then
...
`~/.steez/` (user config directory, not project files). The review dashboard
depends on this data.

```bash
~/.steez/bin/review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
