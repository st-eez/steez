...
Delivery mechanism: spawn.sh pastes the escaped command line into the target pane via `tmux load-buffer` + `paste-buffer` (the same primitive `agent-deliver` uses for in-session messages), not via `send-keys`. That matters for long prompts — `send-keys` types keystrokes through the target pane's TTY line buffer, which caps around 4 KB, so multi-KB briefs would get truncated. The paste-buffer path writes the whole command in one operation and has no practical size ceiling. As a caller you don't need to care about the mechanism, only the consequence: prompt length is not a concern, pass whatever brief the task needs.
